...
def make_localizer(current_locale_name, translation_directories):
    """
    This is a copy of pyramid's  make_localizer, only it changes the
    translation domain from formalchemy to faapp, so the formalchemy strings
    can be found by localizer too.
...
def get_localizer(request):
    """
    An exact copy of pyramid's get_localizer, only it calls local
    make_localizer.
